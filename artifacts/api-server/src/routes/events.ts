...
router.get("/events", async (req, res): Promise<void> => {
  ...
  const rows = await db
    .select({
      ...
      linkOrganizzatore: eventsTable.linkOrganizzatore,
      parentId: eventsTable.parentId,
      tags: eventsTable.tags,
      artisti: eventsTable.artisti,
      socialContatti: eventsTable.socialContatti,
...
router.get("/events/:id", async (req, res): Promise<void> => {
  ...
  res.json(
    GetEventResponse.parse({
      ...
      link_organizzatore: row.linkOrganizzatore,
      parent_id: row.parentId,
      tags: row.tags || [],
      artisti: row.artisti || [],
      social_contatti: row.socialContatti || [],
